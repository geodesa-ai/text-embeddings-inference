...
use chalice::cuda_backend::cudarc::driver::sys::CUdevice_attribute::{
...
pub fn get_runtime_compute_cap() -> Result<usize, anyhow::Error> {
    ...
    let device = driver::result::device::get(0).context("CUDA is not available")?;
    let major = unsafe {
        driver::result::device::get_attribute(device, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR)
    }
    .context("Could not retrieve device compute capability major")?;
    let minor = unsafe {
        driver::result::device::get_attribute(device, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR)
    }
