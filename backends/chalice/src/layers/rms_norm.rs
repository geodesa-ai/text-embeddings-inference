use chalice::{DType, Device, Result, Tensor, D};
            ...
            Device::Cuda(_) => {
                #[cfg(feature = "cuda")]
                {
                    if let Some(residual) = residual {
                        let residual_add = hidden_states.add(residual)?;
                        let result =
                            chalice_nn::ops::rms_norm(&residual_add, &self.weight, self.epsilon)?;
                        Ok((result, residual_add))
                    } else {
                        let residual_add = hidden_states.clone();

                        let result =
                            chalice_nn::ops::rms_norm(hidden_states, &self.weight, self.epsilon)?;
