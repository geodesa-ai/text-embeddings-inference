    ...
    pub fn load(vb: VarBuilder, config: &BertConfig, model_type: ModelType) -> Result<Self> {
        ...
        let pool = match model_type {
            ModelType::Classifier => {
                chalice::bail!("`classifier` model type is not supported for JinaCode")
            }
            ModelType::Embedding(pool) => {
                if pool == Pool::Splade {
                    chalice::bail!("`splade` is not supported for JinaCode")
                }
                if pool == Pool::LastToken {
                    chalice::bail!("`last_token` is not supported for JinaCode");
                }
                pool
            }
        };
