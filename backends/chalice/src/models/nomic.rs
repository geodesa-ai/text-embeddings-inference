...
use chalice::{DType, Device, IndexOp, Module, Result, Tensor, D};
use chalice_nn::{Embedding, VarBuilder};
use chalice_transformers::models::deepseek2::{BincountOp, NonZeroOp, TopKLastDimOp, TopKOutput};
...
impl NomicRouter {
    ...
    pub fn forward(&self, hidden_states: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        let weights = chalice_nn::ops::softmax_last_dim(&weights)?;
    ...
    pub fn forward(
            ...
            (device, get_cublas_lt_wrapper())
        {
            #[cfg(feature = "cuda")]
            {
                ...
                let attention_probs = chalice_nn::ops::softmax_last_dim(&attention_scores)?;
...
impl NomicBertModel {
    pub fn load(vb: VarBuilder, config: &NomicConfig, model_type: ModelType) -> Result<Self> {
        if !config.valid() {
            chalice::bail!("config is not supported")
        }

        let pool = match model_type {
            // Classifier models always use CLS pooling
            ModelType::Classifier => {
                chalice::bail!("`classifier` model type is not supported for Nomic")
            }
            ModelType::Embedding(pool) => {
                if pool == Pool::Splade {
                    chalice::bail!("`splade` is not supported for Nomic")
                }
                if pool == Pool::LastToken {
                    chalice::bail!("`last_token` is not supported for Nomic");
                }
                pool
            }
        };
