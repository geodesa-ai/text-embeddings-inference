...
use chalice::{DType, Device, IndexOp, Module, Result, Shape, Tensor, D};
    ...
    pub fn load(vb: VarBuilder, config: &MPNetConfig, model_type: ModelType) -> Result<Self> {
        let pool = match model_type {
            ...
            ModelType::Classifier => {
                chalice::bail!("`classifier` model type is not supported for MPNet")
            }
            ModelType::Embedding(pool) => {
                if pool == Pool::Splade {
                    chalice::bail!("`splade` is not supported for MPNet")
                }
                pool
            }
        };
