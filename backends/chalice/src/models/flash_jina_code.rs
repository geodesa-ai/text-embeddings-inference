...
use chalice::{DType, Device, IndexOp, Result, Tensor};
use chalice_nn::VarBuilder;
...
impl JinaCodeAttention {
    ...
    pub fn forward(
        ...
        let query_layer = &qkv[0].flatten_from(chalice::D::Minus2)?;
        let key_layer = &qkv[1].flatten_from(chalice::D::Minus2)?;
        ...
        let attention = attention.flatten_from(chalice::D::Minus2)?;
...
impl FlashJinaCodeBertModel {
    pub fn load(vb: VarBuilder, config: &BertConfig, model_type: ModelType) -> Result<Self> {
        let alibi = match config.position_embedding_type {
            ...
            _ => chalice::bail!("not supported"),
        };

        match vb.device() {
            Device::Cuda(_) => {}
            _ => chalice::bail!("FlashJinaCodeBertModel requires Cuda"),
        }

        if vb.dtype() != DType::F16 {
            chalice::bail!("FlashJinaCodeBertModel requires DType::F16")
        }

        let pool = match model_type {
            ModelType::Classifier => {
                chalice::bail!("`classifier` model type is not supported for Jina Code")
            }
            ModelType::Embedding(pool) => {
                if pool == Pool::Splade {
                    chalice::bail!("`splade` is not supported for Jina Code")
                }
                pool
            }
        };
