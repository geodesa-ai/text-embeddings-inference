            ...
            Device::Cuda(_) => {
                #[cfg(feature = "cuda")]
                {
                    ...
                    if let Some(residual) = residual {
                        let residual_add = hidden_states.add(residual)?;
                        let result =
                            chalice_nn::ops::rms_norm(&residual_add, &weight, self.epsilon)?;
                        Ok((result, residual_add))
                    } else {
                        let residual_add = hidden_states.clone();

                        let result =
                            chalice_nn::ops::rms_norm(hidden_states, &weight, self.epsilon)?;
...
impl Gemma3Attention {
    ...
    pub fn forward(
            ...
            (device, get_cublas_lt_wrapper())
        {
            ...
            let attn_weights = chalice_nn::ops::softmax_last_dim(&attn_weights)?;
    ...
    pub fn load(vb: VarBuilder, config: &Gemma3Config, model_type: ModelType) -> Result<Self> {
        let pool = match model_type {
            ModelType::Classifier => {
                chalice::bail!("`classifier` model type is not supported for Gemma3")
            }
