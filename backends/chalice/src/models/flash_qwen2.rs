...
impl Qwen2Attention {
    pub fn load(vb: VarBuilder, config: &Qwen2Config) -> Result<Self> {
        if config.use_sliding_window {
            chalice::bail!("Sliding window is not supported for Qwen2",);
        }
...
impl FlashQwen2Model {
    pub fn load(vb: VarBuilder, config: &Qwen2Config, model_type: ModelType) -> Result<Self> {
        match vb.device() {
            Device::Cuda(_) => {}
            _ => chalice::bail!("FlashQwen2 requires Cuda"),
        }

        if vb.dtype() != DType::F16 {
            chalice::bail!("FlashQwen2 requires DType::F16")
        }

        let pool = match model_type {
            ModelType::Classifier => {
                chalice::bail!("`classifier` model type is not supported for Qwen2")
            }
