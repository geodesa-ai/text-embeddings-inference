...
use chalice_nn::{Embedding, Module, VarBuilder};
...
impl FlashGTEModel {
    pub fn load(vb: VarBuilder, config: &GTEConfig, model_type: ModelType) -> Result<Self> {
        match vb.device() {
            Device::Cuda(_) => {}
            _ => chalice::bail!("FlashGTE requires Cuda"),
        }

        if vb.dtype() != DType::F16 {
            chalice::bail!("FlashGTE requires DType::F16")
        }

        if config.logn_attention_clip1 {
            chalice::bail!("`logn_attention_clip1` is not supported");
        }
        if config.logn_attention_scale {
            chalice::bail!("`logn_attention_scale` is not supported");
        }

        if config.position_embedding_type != PositionEmbeddingType::Rope {
            chalice::bail!("Only `PositionEmbeddingType::Rope` is supported");
        }
...
impl Model for FlashGTEModel {
    ...
    fn predict(&self, batch: Batch) -> Result<Tensor> {
        match &self.classifier {
            None => chalice::bail!("`predict` is not implemented for this model"),
            Some(classifier) => {
