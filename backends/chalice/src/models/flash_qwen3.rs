...
impl Qwen3Attention {
    pub fn load(vb: VarBuilder, config: &Qwen3Config) -> Result<Self> {
        if config.use_sliding_window {
            chalice::bail!("Sliding window is not supported for Qwen3");
        }
...
impl FlashQwen3Model {
    pub fn load(vb: VarBuilder, config: &Qwen3Config, model_type: ModelType) -> Result<Self> {
        match vb.device() {
            Device::Cuda(_) => {}
            _ => chalice::bail!("FlashQwen3 requires Cuda"),
        }

        if vb.dtype() != DType::F16 {
            chalice::bail!("FlashQwen3 requires DType::F16")
        }

        let pool = match model_type {
            ModelType::Classifier => {
                chalice::bail!("`classifier` model type is not supported for Qwen3")
            }
