...
impl ChaliceBackend {
    pub fn new(
        ...
        // Get chalice device
        let device = if chalice::utils::cuda_is_available() {
            #[cfg(feature = "cuda")]
            ...
            Ok(Device::Cpu)
        } else if chalice::utils::metal_is_available() {
            Device::new_metal(0)
        } else {
            Ok(Device::Cpu)
        }
        ...
        let model: Result<Box<dyn Model + Send>, BackendError> = match (config, &device) {
            ...
            (Config::Bert(config), Device::Cuda(_)) => {
                if cfg!(feature = "flash-attn")
                    && dtype == DType::F16
                    // Allow disabling FlashAttention for compatibility diagnostics.
...
impl Backend for ChaliceBackend {
...
impl<O> WrapErr<O> for Result<O, chalice::Error> {
    fn s(self) -> Result<O, BackendError> {
