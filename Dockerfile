...
RUN --mount=type=secret,id=actions_results_url,env=ACTIONS_RESULTS_URL \
    ...
    cargo chef cook --release --features ort,chalice,mkl,static-linking --no-default-features --recipe-path recipe.json && sccache -s
...
RUN --mount=type=secret,id=actions_results_url,env=ACTIONS_RESULTS_URL \
    --mount=type=secret,id=actions_runtime_token,env=ACTIONS_RUNTIME_TOKEN \
    cargo build --release --bin text-embeddings-router --features ort,chalice,mkl,static-linking,http --no-default-features && sccache -s
...
RUN --mount=type=secret,id=actions_results_url,env=ACTIONS_RESULTS_URL \
    --mount=type=secret,id=actions_runtime_token,env=ACTIONS_RUNTIME_TOKEN \
    cargo build --release --bin text-embeddings-router --features ort,chalice,mkl,static-linking,grpc --no-default-features && sccache -s
